      ...
      {holdings.length === 0 ? (
        ...
        <>
          ...
          <div className="md:hidden space-y-4">
            {holdings.map((row) => (
              <div key={row.id} className="rounded-2xl border border-white/5 bg-white/[0.02] p-5 flex flex-col gap-5">
                {/* Header */}
                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                  <div className="flex items-center gap-3">
                    <div className="p-2 rounded-xl bg-white/5 border border-white/10">
                      <row.icon className="h-5 w-5 text-white/80" />
                    </div>
                    <div>
                      <p className="font-bold text-white text-base">{row.asset}</p>
                      <p className="text-[10px] text-white/20 font-mono tracking-tighter">#{row.id.slice(0, 8)}</p>
                    </div>
                  </div>
